...
def gaussian_gradients(xyzs, means, covs):
    """
    xyzs:  [M, 3] - query points (where gradient is computed)
    means: [N, 3] - centers of Gaussian functions
    covs:  [N, 6] - upper-triangle elements of covariance matrices for each Gaussian

    returns: [M, N, 3] - gradients of unnormalized Gaussian values
             grad[m, n] = ∇_{xyzs[m]} exp(-1/2 (xyzs[m] - means[n])^T Σ_n^{-1} (xyzs[m] - means[n]))
    """
    M = xyzs.shape[0]
    N = means.shape[0]

    x_i = xyzs[:, None, :]   # [M, 1, 3]
    mu_j = means[None, :, :] # [1, N, 3]
    diff = x_i - mu_j        # [M, N, 3]
    x, y, z = diff[..., 0], diff[..., 1], diff[..., 2]

    # Extract and prepare inverse covariance elements
    a, b, c, d, e, f = (
        covs[:, 0],
        covs[:, 1],
        covs[:, 2],
    ...
    weights = torch.exp(power)  # unnormalized weights [M, N]

    # Gradient: -w * inv_cov @ diff
    dx = -(x * inv_a + y * inv_b + z * inv_c) * weights
    dy = -(x * inv_b + y * inv_d + z * inv_e) * weights
    dz = -(x * inv_c + y * inv_e + z * inv_f) * weights

    grad = torch.stack([dx, dy, dz], dim=-1)  # [M, N, 3]
    return weights, grad

def compute_normalized_field_gradient(F: torch.Tensor, grad_F: torch.Tensor):
    """
    Compute the spatial gradient of a normalized vector field F' = F / ||F||
    
    Args:
        F: Tensor of shape (B, 3), the vector field at each point
        grad_F: Tensor of shape (B, 3, 3), the spatial gradient (Jacobian) of F at each point
                grad_F[b] is a 3x3 matrix where (i,j) = ∂F_i / ∂x_j
    
    Returns:
        grad_F_normalized: Tensor of shape (B, 3, 3), the gradient of normalized field F'
    """
    B = F.shape[0]
    
    norm_F = torch.norm(F, dim=1, keepdim=True) + 1e-8  # (B, 1)
    F_normalized = F / (norm_F)                   # (B, 3)

    # Compute (F_normalized^T @ grad_F): shape (B, 3)
    # Each is a row vector: shape (B, 1, 3) x (B, 3, 3) -> (B, 1, 3)
    FnT_gradF = torch.bmm(F_normalized.unsqueeze(1), grad_F).squeeze(1)  # (B, 3)

    # Outer product: F_normalized (B, 3) ⊗ FnT_gradF (B, 3) -> (B, 3, 3)
    outer = torch.bmm(F_normalized.unsqueeze(2), FnT_gradF.unsqueeze(1))  # (B, 3, 3)

    grad_F_normalized = (grad_F - outer) / norm_F.unsqueeze(-1)  # broadcast to (B, 3, 3)
...
class GaussianBaseModel(BaseGeometry, GaussianIO):
    ...
    class Config(BaseGeometry.Config):
        ...
        another_feature: bool = False
        another_feature_dim: int = 3
        another_feature_lr: float = 0.001
        
        feature_similiarity_thld: float = 0.9
        
        remove_instances_outlier_from_iter: int = 99999999
        remove_instances_outlier_until_iter: int = 99999999
        remove_instances_outlier_interval: int = 99999999
    ...
    def setup_functions(self):
        ...
        self.antoher_feature_activation = torch.nn.functional.normalize
    ...
    def configure(self) -> None:
        ...
        if self.cfg.another_feature:
            self._another_feature = torch.empty(0)
        self.optimizer = None
    ...
    @property    
    def get_another_feature(self):
        if self.cfg.another_feature:
            return self.antoher_feature_activation(self._another_feature)
        else:
            raise ValueError("Another feature is not predicted")
        
    @property
    def get_instance_masks(self):
        return self.instance_masks.bool().detach()
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        if self.cfg.another_feature:
            another_feature = torch.randn((fused_point_cloud.shape[0], self.cfg.another_feature_dim), device="cuda")
            self._another_feature = nn.Parameter(another_feature.requires_grad_(True))
    ...
    def training_setup(self):
        ...
        if self.cfg.another_feature:
            l.append(
                {
                    "params": [self._another_feature],
                    "lr": C(training_args.another_feature_lr, 0, 0),
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "another_feature":
                param_group["lr"] = C(
                    self.cfg.another_feature_lr, 0, iteration, interpolation="exp"
                )
    ...
    def to(self, device="cpu"):
        ...
        if self.cfg.another_feature:
            self._another_feature = self._another_feature.to(device)
    ...
    def prune_points(self, mask):
        ...
        if self.cfg.another_feature:
            self._another_feature = optimizable_tensors["another_feature"]
        if self.instance_masks is not None:
            self.instance_masks = nn.Parameter(self.instance_masks[valid_points_mask])
    ...
    def densification_postfix(
        ...
        new_rotation,
        new_normal=None,
        new_another_feature=None,
        new_instance_masks=None,
    ):
        d = {
            "xyz": new_xyz,
        ...
        if self.cfg.another_feature:
            d.update({"another_feature": new_another_feature})
        ...
        if self.cfg.another_feature:
            self._another_feature = optimizable_tensors["another_feature"]
        if new_instance_masks is not None:
            self.instance_masks = nn.Parameter(torch.cat([self.instance_masks, new_instance_masks], dim=0))
    ...
    def densify_and_split(self, grads, grad_threshold, N=2):
        ...
        if self.cfg.pred_normal:
            ...
            new_normal = None
        if self.cfg.another_feature:
            new_another_feature = self._another_feature[selected_pts_mask].repeat(N, 1)
        else:
            new_another_feature = None
        if self.instance_masks is not None:
            new_instance_masks = nn.Parameter(self.instance_masks[selected_pts_mask].repeat(N, 1))
        else:
            new_instance_masks = None
    ...
    def densify_and_clone(self, grads, grad_threshold):
        ...
        if self.cfg.pred_normal:
            ...
            new_normal = None
        if self.cfg.another_feature:
            new_another_feature = self._another_feature[selected_pts_mask]
        else:
            new_another_feature = None
        if self.instance_masks is not None:
            new_instance_masks = nn.Parameter(self.instance_masks[selected_pts_mask])
        else:
            new_instance_masks = None
    ...
    def add_instance_densification_stats(self, instancewise_viewspace_point_tensor, instancewise_update_filter, width, height):
        num_instance = len(instancewise_viewspace_point_tensor)
        for i in range(num_instance):
            viewspace_point_tensor = instancewise_viewspace_point_tensor[i]
            update_filter = self.get_instance_masks[:, i].clone()
            update_filter[update_filter.clone()] = instancewise_update_filter[i]
            if width is None or height is None:
                self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter, :2], dim=-1, keepdim=True)
                self.denom[update_filter] += 1
            else:
                ...
                grad[:, 0] *= width * 0.5
                grad[:, 1] *= height * 0.5
                self.xyz_gradient_accum[update_filter] += torch.norm(grad[instancewise_update_filter[i],:2], dim=-1, keepdim=True)
                self.denom[update_filter] += 1
    ...
    def update_states(
        ...
        instancewise_visibility_filter = None,
        instancewise_viewspace_points = None,
        ...
        for i in range(bs):
            ...
            self.add_densification_stats(viewspace_point_tensor_i, visibility_filter_i, width, height)
        
        for instancewise_visibility_filter_i, instancewise_viewspace_points_i in zip(instancewise_visibility_filter, instancewise_viewspace_points):
            self.add_instance_densification_stats(instancewise_viewspace_points_i, instancewise_visibility_filter_i, width, height)
        ...
        if (
            iteration > self.cfg.remove_instances_outlier_from_iter
            and iteration < self.cfg.remove_instances_outlier_until_iter
            and self.instance_masks is not None
            and iteration % self.cfg.remove_instances_outlier_interval == 0
        ):
            self.remove_instances_outlier_points()
        ...
        if (
            ...
            densify_grad_threshold = C(self.cfg.densify_grad_threshold, 0, iteration)
            self.densify(densify_grad_threshold)
            
    @torch.no_grad()
    def classify_by_feature(self, features):
        if self.instance_masks is None:
            instance_masks = self.get_another_feature @ features.T > self.cfg.feature_similiarity_thld
            opacity_masks = self.get_opacity > 0.9
            instance_masks = instance_masks * opacity_masks
            self.instance_masks = nn.Parameter(instance_masks.float())
            prune_mask = instance_masks.sum(dim=-1) == 0
            self.prune_points(prune_mask)
            
    @torch.no_grad()
    def get_bboxs_of_instances(self):
        opacity_masks = self.get_opacity > 0.9
        bbox_min = []
        bbox_max = []
        for mask in self.get_instance_masks.T:
            masked_xyz = self.get_xyz[mask*opacity_masks.squeeze()]
            bbox_min.append(masked_xyz.min(dim=0).values)
            bbox_max.append(masked_xyz.max(dim=0).values)
        bbox_min = torch.stack(bbox_min)
        bbox_max = torch.stack(bbox_max)
        return bbox_min, bbox_max
    
    @torch.no_grad()
    def remove_instances_outlier_points(self):
        prune_mask = torch.zeros_like(self.get_opacity).bool().squeeze()
        for instance_mask in self.get_instance_masks.T:
            masked_xyz = self.get_xyz[instance_mask]
            clustering = DBSCAN(eps=self.cfg.cluster_eps, min_samples=1).fit(masked_xyz.detach().cpu().numpy())
            cluster_label, counts = np.unique(clustering.labels_, return_counts=True)
            instance_prune_mask = clustering.labels_ != cluster_label[counts.argmax()]
            instance_prune_mask = torch.from_numpy(instance_prune_mask).to(prune_mask)
            prune_mask[instance_mask] = instance_prune_mask
        self.prune_points(prune_mask)
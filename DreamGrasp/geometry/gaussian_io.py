...
class GaussianIO:
    def construct_list_of_attributes(self):
        ...
        if self.cfg.another_feature:
            for i in range(self._another_feature.shape[1]):
                l.append("af_{}".format(i))
        return l
    ...
    def save_ply(self, path):
        ...
        attributes_tuple = (xyz, normals, f_dc, f_rest, opacities, scale, rotation)
        if self.cfg.another_feature:
            another_feature = self._another_feature.detach().cpu().numpy()
            attributes_tuple += (another_feature, )
        attributes = np.concatenate(attributes_tuple, axis=1)
    ...
    def load_ply(self, path):
        ...
        if self.cfg.another_feature:
            another_feature = np.zeros((xyz.shape[0], self.cfg.another_feature_dim))
            for i in range(self.cfg.another_feature_dim):
                another_feature[:, i] = np.asarray(plydata.elements[0]["af_{}".format(i)])
            self._another_feature = nn.Parameter(
                torch.tensor(another_feature, dtype=torch.float, device="cuda").requires_grad_(True)
            )
    ...
    @torch.no_grad()
    def extract_instance_mesh(self, resolution=128, num_blocks=16, relax_ratio=1.5, density_thresh=0.8, decimate_target=1e5):
        # resolution: resolution of field
        ...
        for instance_idx in range(self.get_instance_masks.shape[-1]):
            instance_mask = self.get_instance_masks[:, instance_idx]
            opacity_mask = (self.get_opacity > 0.005).squeeze(1)
            mask = instance_mask * opacity_mask

            # pre-filter low opacity gaussians to save computation

            opacities = self.get_opacity[mask]
            xyzs = self.get_xyz[mask]
            stds = self.get_scaling[mask]
            ...
            mesh = Mesh(v_pos=v, t_pos_idx=f)
            instance_mesh_list.append(mesh)
            
        return instance_mesh_list
